...
public class WodeFragment extends BaseFragment implements IWodeView {
    ...
    MyItemView person_center_balance_num;//我的余额
    @CodeNote(id = R.id.person_center_package_num)
    MyItemView person_center_package_num;//礼包
    @CodeNote(id = R.id.person_center_integral_num)
    MyItemView person_center_integral_num;//积分
    ...
    private void setText(WoDeModel model) {
        person_center_balance_num.setRightNumber(model.getCash() + "");
        person_center_package_num.setRightNumber(model.getGift() + "");
        person_center_integral_num.setRightNumber(model.getPoint() + "");
